...
def convert_image_to_webp(input_path, output_path=None, quality=85, optimize=True, max_width=1920, max_height=1920):
    """
    Convertit un fichier PNG ou JPEG en WebP avec optimisation
    ...
    Args:
        ...
        max_width (int): Largeur maximale (1920 par défaut)
        max_height (int): Hauteur maximale (1920 par défaut)
        ...
        with Image.open(input_path) as img:
            ...
            if not is_logo:
                ...
                if width > max_width or height > max_height:
                    ...
                    print(f"  Image redimensionnee: {original_size_pixels} -> {img.size}")
            
            # Déterminer le chemin de sortie
            if output_path is None:
                output_path = str(Path(input_path).with_suffix('.webp'))
            
            # Sauvegarder en WebP avec méthode optimale
            img.save(output_path, 'WebP', quality=quality, method=6, optimize=optimize)
...
def main():
    ...
    parser.add_argument("--max-width", type=int, default=1920,
                       help="Largeur maximale pour redimensionner (défaut: 1920)")
    parser.add_argument("--max-height", type=int, default=1920,
                       help="Hauteur maximale pour redimensionner (défaut: 1920)")
